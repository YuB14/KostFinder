...
class _FavoriteScreenState extends State<FavoriteScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final aktif = _favs.where((f) => f.status.toLowerCase() == 'aktif').length.toString();
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text('Kost Favorit ❤️', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.white)),
                          Text('Daftar kost yang kamu simpan', style: TextStyle(fontSize: 11, color: Colors.white70)),
                        ]),
                      ]),
                    ]),
                  ),
              ...
              title: const Text('Favorit ❤️', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                // Stat cards
                Row(children: [
                  _StatCard(icon: Icons.favorite_rounded, value: totalFav, label: 'Total Favorit', accentColor: AppColors.coral, accentBg: AppColors.coralBg),
                  const SizedBox(width: 10),
                  _StatCard(icon: Icons.emoji_events_rounded, value: terbaru, label: 'Terbaru', accentColor: AppColors.teal, accentBg: AppColors.tealBg),
                  const SizedBox(width: 10),
                  _StatCard(icon: Icons.check_circle_rounded, value: aktif, label: 'Kost Aktif', accentColor: AppColors.yellow, accentBg: AppColors.yellowBg),
                ]),
                const SizedBox(height: 20),
                Text('Daftar Kost Favorit', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: textColor)),
          ...
          if (_isLoading)
            ...
            SliverFillRemaining(
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 12),
                Text('Belum ada kost favorit', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: isDark ? AppColors.textDark : AppColors.textLight)),
                const SizedBox(height: 8),
                Text('Cari kost dan simpan yang kamu suka!', style: TextStyle(color: isDark ? AppColors.mutedDark : AppColors.mutedLight, fontSize: 13)),
              ]),
            )
          else
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                  (_, i) => _FavCard(data: _favs[i], isDark: isDark, onToggle: () => _toggleFav(i)),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({required this.icon, required this.value, required this.label, required this.accentColor, required this.accentBg});
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          Text(value, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: textColor), maxLines: 1, overflow: TextOverflow.ellipsis),
...
class _FavCard extends StatelessWidget {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Stack(children: [
            if (data.foto != null && data.foto!.isNotEmpty && !data.foto!.toLowerCase().contains('default'))
              Positioned.fill(child: Image.network(data.foto!, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Center(child: Icon(Icons.home_rounded, color: muted, size: 36))))
            else
              Center(child: Icon(Icons.home_rounded, color: muted, size: 36)),
            Positioned(
              top: 8, right: 8,
  ...
  Widget _statusBadge(String text, String type, bool isDark, Color muted, Color bg2) {
    Color color, bg;
    switch (type) {
      case 'blue': color = AppColors.blue; bg = AppColors.blueBg; break;
      case 'teal': color = AppColors.teal; bg = AppColors.tealBg; break;
      case 'yellow': color = AppColors.yellow; bg = AppColors.yellowBg; break;
      case 'muted': color = muted; bg = bg2; break;
      case 'green':
      default: color = AppColors.green; bg = AppColors.greenBg; break;
    }
    return Container(
...
class _FavData {
  ...
  final bool isFav;
  const _FavData({required this.id, this.foto, required this.name, required this.location, required this.price, required this.status, required this.statusType, required this.hearts, required this.isFav});
  _FavData copyWith({bool? isFav}) => _FavData(id: id, foto: foto, name: name, location: location, price: price, status: status, statusType: statusType, hearts: hearts, isFav: isFav ?? this.isFav);
}