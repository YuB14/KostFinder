...
class LandingController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($keyword)) {
            // Find related wilayah IDs based on keyword
            $wilayahIds = Wilayah::where('nama_wilayah', 'regex', '/' . preg_quote($keyword, '/') . '/i')
                ->pluck('_id')
                ->toArray();
                
            // Check if keyword contains numbers (for price search)
            $numericValue = preg_replace('/[^0-9]/', '', $keyword);

            $query->where(function ($q) use ($keyword, $wilayahIds, $numericValue) {
                // Match nama_kost
                $q->where('nama_kost', 'regex', '/' . preg_quote($keyword, '/') . '/i');
                
                // Or match wilayah
                if (!empty($wilayahIds)) {
                    $q->orWhereIn('wilayah_id', $wilayahIds);
                }
                
                // Or match harga_kost (budget <= numeric input)
                if (!empty($numericValue) && is_numeric($numericValue)) {
                    $q->orWhere('harga_kost', '<=', (int)$numericValue);
                }
            });
        }
