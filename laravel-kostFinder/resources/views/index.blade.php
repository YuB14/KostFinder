...
<section class="hero">
  ...
  <form class="search-box" id="search-form" onsubmit="searchKost(event)">
    <span>📍</span>
    <input type="text" id="search-nama" name="nama" placeholder="Cari nama kost..." autocomplete="off"/>
    <select id="search-wilayah" name="wilayah_id">
      <option value="">Semua Wilayah</option>
      @foreach($wilayahList as $w)
        <option value="{{ $w->_id }}">{{ $w->nama_wilayah }}</option>
      @endforeach
    </select>
...
<script>
  ...
  async function searchKost(e) {
    ...
    const wilayahId = document.getElementById('search-wilayah').value;

    if (!nama && !wilayahId) {
      alert('Masukkan nama kost atau pilih wilayah untuk mencari.');
      return;
    }
